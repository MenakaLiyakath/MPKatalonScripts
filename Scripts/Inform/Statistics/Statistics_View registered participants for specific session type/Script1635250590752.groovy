...
for (i = 1; i < totSess_ManageSession; i++) {
    ...
    def sessinpg = sessiontypeonsessionpage.toString()

    def sessionvar = SessType.toString()

    if (sessionvar.equalsIgnoreCase(sessinpg)) {
        //if(sessiontypeonsessionpage.equalsIgnoreCase(sessiontype))
...
WebUI.click(findTestObject('module_inform/image_inform_sessionTypeDropdownArrow'))

WebUI.delay(2)

def dropXpath = '//span[@class=\'ng-option-label\']'

CustomKeywords.'verificationFunctions.droppanelselection.dropValueSelection'(dropXpath, SessType)
