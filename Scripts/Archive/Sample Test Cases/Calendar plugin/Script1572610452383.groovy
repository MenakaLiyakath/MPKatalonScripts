...
/*if (month != '') {
    ...
    int startMonthInt = CustomKeywords.'customOmegaKeywords.customKeywordsOmega.pickCalendarDate'(month)

    CustomKeywords.'com.katalon.plugin.keyword.calendar.SetDateCalendarKeyword.setDate'(findTestObject('Encounter(SessionOrRetreat)/Retreat/page_retreatOverview/calendar_overview_startDAte'), 
        day.toInteger(), startMonthInt, year.toInteger(), 2000, FailureHandling.CONTINUE_ON_FAILURE) //Calendar plugin Implementation Ends here- card 768
}*/
...
if ((isOvernight == 'true') && (numberOfDays > 1)) {
    ...
    /*    calendarDateVariable = ('Retreats/Retreat-Overview-Page/calendarDates/calendar-date-' + day)
...
/*WebUI.verifyElementVisible(findTestObject('Encounter(SessionOrRetreat)/Retreat/page_retreatOverview/alert_Overview_saveSuccessfulMessage'), 
    FailureHandling.CONTINUE_ON_FAILURE)*/
...
/*WebUI.verifyElementPresent(findTestObject('Encounter(SessionOrRetreat)/Retreat/page_retreatOverview/alert_Overview_saveSuccessfulMessage'), 
    0, FailureHandling.CONTINUE_ON_FAILURE)*/
...
'QA note: might be better to name variables instead of using generic checkValue?'
actualDayOneStartHour = WebUI.getAttribute(findTestObject('Encounter(SessionOrRetreat)/Retreat/page_retreatOverview/retreat_startTimeDay1/input_overview_startTimeHours_day1'), 
    'value', FailureHandling.CONTINUE_ON_FAILURE)
